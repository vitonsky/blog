...
The mainstream technologies to build user interface does not limit programmer in a ways to implement design, so it is simple to build inconsistent and unmaintainable code base that won't scale. One of the popular ways to do it is to use in-place styling.
