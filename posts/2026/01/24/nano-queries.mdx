...
If your app are local-first and you use some database on client side instead of indexedDB, you may build queries right in browser right now. No drivers needed.

If you have a backend on nodejs, now you have an option to write the performant SQL queries with no suffer with a raw strings. Nano queries stores raw SQL and values in boxes, so it will never be mixed.
